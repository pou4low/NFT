(() => {
    ...
    if (document.querySelector(".timer")) {
        ...
        const nextDays = new Date(`February 04 2024 0:00:00`);
        function updateCounter() {
            const currentTime = new Date;
            const diff = nextDays - currentTime;
            let hoursLeft = Math.floor(diff / 1e3 / 60 / 60) % 24;
            let minutesLeft = Math.floor(diff / 1e3 / 60) % 60;
            let secondsLeft = Math.floor(diff / 1e3) % 60;
